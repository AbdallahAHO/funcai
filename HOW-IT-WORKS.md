...
| `pnpm models:report` | Summarize model registry changes versus `HEAD` |
...
The scheduled `Refresh Models` GitHub Actions workflow runs weekly, regenerates the registry, and opens or updates a maintenance PR with an explicit added/removed/updated model summary generated by `pnpm models:report`. For local workflow validation with `act`, this repo pins the full GitHub runner image in `.actrc`.
